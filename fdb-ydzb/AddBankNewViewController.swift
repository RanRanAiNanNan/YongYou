//
//  AddBankNewViewController.swift
//  fdb-ydzb
//
...
class AddBankNewViewController: BaseTableViewController,choseBankDelegate,ProvinceNameDelegate,CityNameDelegate,UITextFieldDelegate {
    ...
    func addBandCardClick(){
        ...
        if self.cardNoTf.text != self.reCardNoTf.text {
            KGXToast.showToastWithMessage("您输入的两次银行卡号不一致", duration: ToastDisplayDuration.LengthShort)
            return
        }
