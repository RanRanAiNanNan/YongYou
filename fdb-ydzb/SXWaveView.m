...
#define MOVE_ACTION  self.bounds.size.width / 2           
...
- (instancetype)initWithFrame:(CGRect)frame
{
    if (self = [super initWithFrame:frame]) {
        ...
        avgScoreLbl.centery = rotateImg.centery - W/125;
        avgScoreLbl.font = [UIFont systemFontOfSize:25*W/125];
...
- (void)setPrecent:(float)precent{
    ...
    NSString *formattedNumberString = [numberFormatter stringFromNumber:[NSNumber numberWithFloat:_precent]];
    NSString *text = [formattedNumberString stringByAppendingString:@"%"];
    self.avgScoreLbl.text = text;
}

- (void)setPrecent:(float)precent textColor:(UIColor *)tcolor alpha:(CGFloat)alpha
{
    [self setAlpha:alpha];
    [self setTextColor:tcolor];
    [self setPrecent:precent];
}
- (void)setPrecent:(float)progress avg:(float)precent textColor:(UIColor *)tcolor bgColor:(UIColor *)bColor alpha:(CGFloat)alpha clips:(BOOL)clips{
    [self setAlpha:alpha];
    [self setPrecent:precent];
    [self setClips:clips];
...
- (void)addAnimateWithType:(int)type
{
    ...
    void(^acallBack)(CGFloat start) = ^(CGFloat start) {
        ...
        moveAction.values = [NSArray arrayWithObjects:[NSNumber numberWithFloat:MOVE_ACTION],[NSNumber numberWithFloat:start],nil];
...
//        moveAction.autoreverses = YES;
    ...
    if (type == 0) {
        CGFloat avgScore = self.progress;
