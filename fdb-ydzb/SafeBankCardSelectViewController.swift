...
class SafeBankCardSelectViewController: BaseTableViewController {
    ...
    override func viewDidLoad() {
        ...
        print("本地的数据")
        print(self.mobileArr)
//        zhegaiView.hidden = true
        //加载初始化视图
        initView()
...
//        loadData()
    ...
    override func tableView(tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        ...
        print(self.mobileArr?.count)
//        if self.mobileArr == nil {
//            return 1
//        }else{
...
//        }
    ...
    override func tableView(tableView: UITableView, cellForRowAtIndexPath indexPath: NSIndexPath) -> UITableViewCell {
        ...
        if self.mobileArr != nil {
////            print("row:\(indexPath.row)  type:\(self.bandCardRecordData[indexPath.row].type)")
            ...
            print("*******\(self.mobileArr?.objectAtIndex(indexPath.row).objectForKey("number"))")
            let str = self.mobileArr?.objectAtIndex(indexPath.row ).objectForKey("name")
//            银行名
            let bannerNameLable:UILabel = cell.viewWithTag(201) as! UILabel!
            bannerNameLable.text = str as? String
//
//            
//            //卡号
            let strNum = self.mobileArr?.objectAtIndex(indexPath.row ).objectForKey("number")
            let bannerNoLable:UILabel = cell.viewWithTag(202) as! UILabel!
            bannerNoLable.text = strNum as? String
//
//            
//            
//            //真实姓名
...
//            //默认选择
            selectedImg = cell.viewWithTag(203) as! UIImageView!
            selectedImg!.hidden = true
            selectedImg!.tag = indexPath.row
...
//        }
//        let cell : UITableViewCell = tableView.superview as! UITableViewCell
    
        index = indexPath.row
        tableView.reloadData()
        print(indexPath.row)
        if (selectedImg!.tag == indexPath.row) {
            selectedImg!.hidden = true
        }
        if (selectedImg!.tag != indexPath.row){
            selectedImg!.hidden = false
