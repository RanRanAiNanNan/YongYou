...
-(instancetype)init
{
    self = [super init];
    if (self) {
        self.frame = CGRectMake(0, -44, kScreenWidth, kScreenHeight );
...
-(void)_createView
{
    ...
    _navigationView.backgroundColor = [UIColor colorWithRed:242/255.0 green:244/255.0 blue:245/255.0 alpha:1];
    ...
    for (int i = 0; i <buttonTitleArray.count ; i++) {
        ...
        if (i == 0) {
            [button setTitleColor:[UIColor colorWithRed:91/255.0 green:95/255.0 blue:101/255.0 alpha:1] forState:UIControlStateNormal];
        }else{
            [button setTitleColor:[UIColor colorWithRed:91/255.0 green:98/255.0 blue:197/255.0 alpha:1] forState:UIControlStateNormal];
        }
    ...
    UILabel *laa = [[UILabel alloc]initWithFrame:CGRectMake(kScreenWidth / 2 -30, 0, 60, navigationViewHeight)];
    laa.text = @"选择地区";
    laa.textColor = [UIColor colorWithRed:91/255.0 green:95/255.0 blue:101/255.0 alpha:1];
    laa.font = [UIFont systemFontOfSize:15];
    [_navigationView addSubview:laa];
...
-(void)tapButton:(UIButton*)button
{
    ...
    if (button.tag == 1) {
        ...
        //参数1:事件标识
        //参数2:向接受通知的对象传一个值(对象)
        [center postNotificationName:@"add_address_tongzhi" object:[UIColor redColor] userInfo:@{@"province":province,@"city":city,@"town":town}];
  
    }else{
        NSNotificationCenter *center=[NSNotificationCenter defaultCenter];
        //参数1:事件标识
        //参数2:向接受通知的对象传一个值(对象)
        [center postNotificationName:@"add_address_tong" object:[UIColor redColor] userInfo:nil];
